  ...
  <div class="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-40">
    <div class="flex items-center justify-center gap-4 p-4 bg-white rounded-lg shadow-lg">
      ...
      >
        ...
        <span class="text-xs mt-1">{{ isMuted ? '음소거됨' : '음소거' }}</span>
      </button>
...
const toggleMute = () => {
  ...
  // LiveKit 오디오 트랙이 있으면 음소거 처리
  if (callStore.audioTrack) {
    callStore.audioTrack.setEnabled(!isMuted.value)
  }

  // 이벤트 발생 (부모 컴포넌트에서 처리 가능)
  emit('mute-changed', isMuted.value)
}
